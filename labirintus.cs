...
namespace LabirintusAlpha
{
    class Program
    {
        ...
        static void Játék(string[,] labirintus,string pályanév)
        {
            ...
            bool van = ToplistábaKerülhetE(pályatoplista, stopper.ToString());
            int hely = HolAToplistában(pályatoplista, stopper.ToString());
            if (van==true)
            {
                Console.WriteLine("Kérem a nevedet, mert felkerültél a toplistára!(Szóköz ne legyen benne!)");
                string név = Console.ReadLine();
                string időésnév = stopper.ToString()+":"+név;
        ...
        static string[,] ToplistaPályánként(string pályanév,string[] toplista)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                for (int j = 0; j < 2; j++)
                {
                    pályatoplista[i, j] += seged[i].Split(' ');
                }
            }
